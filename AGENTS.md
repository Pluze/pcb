...
- skill scripts: deterministic or fragile mechanics;
- design documentation: only design-specific intent, evidence, risks, and exceptions.

## Repository invariants

- Keep one self-contained project under `Designs/<Meaningful_Name>/`; keep reusable capabilities under `.agents/skills/`.
...
- Run `.agents/skills/pcb-repository-governance/scripts/check_repository_growth.py` as required by the governance skill. Never bypass hooks with `--no-verify`.
