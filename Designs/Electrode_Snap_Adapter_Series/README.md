...
The active build target is the 17-coupon panel under
[`fabrication/u4/Snap_Adapter_17up_150x100/`](fabrication/u4/Snap_Adapter_17up_150x100/).
Import its five machine files together: `TopLayer.gtl` → TopLayer,
`BottomLayer.gbl` → BottomLayer, `BoardOutline.gm1` → BoardOutline,
`CutInside.gm2` → CutInside, and `DrillPlated.drl` → DrillPlated. Keep the continuous
processing outline separate from the 17 coupon contours. Use the qualified U4
recipe for complete copper removal and double-sided registration, then perform
the non-plated center-hole solder operation described above. Standalone coupon
sources are under `variants/`; the build publishes the panel as the active target.
...
```sh
python3 tools/pcbflow/routing/scripts/generate_circular_contact_board.py \
...
```sh
python3 tools/pcbflow/routing/scripts/generate_circular_contact_panel.py \
  --csv Designs/Electrode_Snap_Adapter_Series/series.csv \
...
```sh
python3 tools/pcbflow/kicad/scripts/export_circuitpro_u4_packages.py \
  export Designs/Electrode_Snap_Adapter_Series --force
python3 tools/pcbflow/kicad/scripts/export_kapton_lightburn_templates.py \
  export Designs/Electrode_Snap_Adapter_Series --force
```
...
`./pcb build --design Electrode_Snap_Adapter_Series` refreshes U4, LightBurn DXF and
