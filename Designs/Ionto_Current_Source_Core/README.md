...
The active board is 27 × 20 mm. All eight footprints and all routed copper are on the front side, with zero vias; the production pour variant adds a front-side GND zone while the matching primary remains routing-only. The placement follows the power/current path from the battery header at the left, through the inductor and R1218 stage, to the stimulation header at the right. The CE jumper sits beside the controller/output area, and the removable RSET socket remains accessible along the lower edge.

Freerouting-derived routing uses 0.25 mm (about 10 mil) for control and local GND,
0.30 mm (about 12 mil) for stimulation, and 0.50 mm (about 20 mil) for BAT+, LX and
the outer GND return. The final 53 front-copper segments have only horizontal,
vertical and 45-degree directions. Explicit GND routes connect every component
without relying on the pour. Total trace length is 113.45 mm; LX is 3.00 mm and
remains local to U1/L1. The project offers approximately
6–20 mil presets, with a 0.1524 mm selectable minimum and 0.20 mm clearance;
actual tracks remain at least 0.25 mm.

For the soldermask-free LPKF U4 prototype, the front GND zone uses 0.50 mm clearance to other-net copper and a 0.50 mm edge inset, with 0.25 mm minimum fill thickness, a 0.50 mm thermal gap and 0.50 mm spokes. Actual copper-removal time still depends on the CircuitPro recipe. Explicit GND routes retain connectivity without the pour. C1 and C2 retain deliberate solid-zone pad connections.

The matching primary PCB is intentionally the routing-only design source and contains no zone. It must pass DRC with zero unconnected items before production generation derives [`variants/Ionto_Current_Source_Core_With_Pour.kicad_pcb`](variants/Ionto_Current_Source_Core_With_Pour.kicad_pcb). The production exporter refills and checks a temporary copy of that 0.50 mm GND-zone definition, leaving both design sources unchanged.

The repo-wide [`repository-defaults.json`](../../.agents/skills/kicad-pcb-layout/references/repository-defaults.json) owns the common route-first workflow, single-sided/zero-via policy, 0.20 mm default clearance and 0.25 mm routing width, wider functional net classes, explicit GND-tree requirement, 0.50 mm pour clearance and edge inset, thermal settings, DRC gates, and automatic dual manufacturing outputs. KiCad project classes use those functional targets. Local GND uses the ordinary class; the established outer return is explicitly 0.50 mm.
...
The 0.50 mm BAT/switching trunk is evaluated for this low-output-current prototype,
not adopted as a universal current rating. Using bulk copper resistivity of about
0.0172 ohm mm²/m, a 10 mm long, 0.50 mm wide, 18 µm thick straight trace is about
19 mΩ. At a conservative 1 A pulse reference it drops about 19 mV and dissipates
about 19 mW; actual average loss depends on RMS current. BAT+ and LX routes remain 0.50 mm throughout. These estimates do not establish temperature rise, copper thickness
or fault-current qualification. Confirm real copper/process quality and loaded
behavior on the bench. Wider traces remain appropriate when a new design needs them.

## Supplier Gerbers and unified build

`./pcb build --design Ionto_Current_Source_Core` refreshes U4, LightBurn DXF and
