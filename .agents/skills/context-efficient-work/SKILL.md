...
When auditing task history, project message text and event counts before emitting results. Exclude image/audio payloads, attachments and nested tool responses explicitly; per-item character limits may not bound the returned content array. Paginate until the requested scope is covered and report coverage separately from token claims.
