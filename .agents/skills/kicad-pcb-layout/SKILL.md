...
### Trace ranges and process limits

Read [references/routing-and-clearance.md](references/routing-and-clearance.md) when selecting or changing widths and pour clearances. The profile separates a selectable minimum, ordinary routing targets, and hand-assembly pour spacing. A 6–20 mil preset range is not an ampacity limit or a promise that every process supports 6 mil. Preserve wider electrically justified routes.

### Soldermask-free milled prototypes

For an in-house milled board that will be hand-soldered without solder mask, treat copper-pour spacing as an assembly constraint, not merely as the minimum electrical clearance. Establish final values from the actual isolation tool, runout, registration, copper condition, and a process coupon. When those measurements are not yet available, use the selected repository profile. For the current U4 hand-assembly profile:

- 0.50 mm zone clearance to copper on other nets;
- 0.50 mm thermal-relief gap and 0.50 mm thermal spoke width;
- 0.25 mm minimum zone feature thickness; and
...
Use Freerouting by default for routing/optimization candidates. For offline front-side,
zero-via boards, use [scripts/freerouting_candidate.py](../../../tools/pcbflow/routing/scripts/freerouting_candidate.py)
and the [CLI contract](references/freerouting-build.md). It stages the project, exports
native DSN, runs a bounded router process, imports SES, enforces the requested width
floor, bevels degree-two right-angle bends, and independently checks DRC and geometry.
It never adopts its own candidate. Check functional/return paths and the returned
renders before replacing the source. A failed candidate requires a changed constraint,
placement, or reviewed local completion; do not rerun it unchanged. Keep routing as a
design step, separate from manufacturing rebuilds. Prefer 45-degree bends; pad and
T-junction geometry must be judged in context.
...
- Generate committed 3D PCB previews with the repository profile's `visual_assets` settings. The default is a strict top, orthogonal, basic-quality PNG with a transparent background and no floor so decorative post-processing cannot obscure copper, silkscreen, or component geometry with hard shadows. If the basic renderer draws filled copper above 3D bodies, use the profile's deterministic same-camera composite: extract only the component pixels from matching basic renders with and without models, then place them over a basic model-hidden render of the actually refilled board. Do not use perspective or a floor for engineering previews; keep any explicitly requested presentation render separate.
