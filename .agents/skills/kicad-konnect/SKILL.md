...
For repository-wide manufacturing validation, output refresh, or release readiness, prefer one invocation of `tools/pcbflow/kicad/scripts/pcb_workflow.py` over model-orchestrated calls to each design and exporter:

```bash
python3 tools/pcbflow/kicad/scripts/pcb_workflow.py plan
python3 tools/pcbflow/kicad/scripts/pcb_workflow.py schematic-validate --design <name>
python3 tools/pcbflow/kicad/scripts/pcb_workflow.py pcb-review --design <name>
python3 tools/pcbflow/kicad/scripts/pcb_workflow.py validate
python3 tools/pcbflow/kicad/scripts/pcb_workflow.py release-ready
python3 tools/pcbflow/kicad/scripts/pcb_workflow.py release-ready --apply
```
...
For multi-stage design changes, read [references/workflow-design.md](references/workflow-design.md) for phase contracts, change-impact routing and evidence boundaries. `pcb-review` preserves sibling project/custom rules and returns uncached artifact receipts; it does not establish schematic parity or human visual approval. Manufacturing exporters preserve the selected design project rules in their staging copies too.
...
When direct MCP invocation is unavailable in the active environment, use `tools/pcbflow/kicad/scripts/konnect_mcp_client.py` as a transport client to the same Konnect server, not as a replacement capability. Prefer one `callseq` per bounded phase:

```bash
python3 tools/pcbflow/kicad/scripts/konnect_mcp_client.py \
...
- `refill_zones` with no IPC: close the editor and use `kicad-cli pcb drc --refill-zones --save-board` when source mutation is intended, or run `tools/pcbflow/kicad/scripts/headless_pcb_review.py` to refill and validate an isolated temporary copy without changing the source. GUI refill is only a final fallback when both supported paths are proven unavailable.
...
Use the [public tool guide](../../../tools/pcbflow/README.md) for command selection.
`./pcb check` performs saved ERC, native-export endpoint/value/footprint agreement
and schematic parts-field inventory. It runs before routing in `finish` and before
manufacturing audit in `verify`; empty schematics report parity as not applicable.
`./pcb finish --design NAME` composes saved-design checks, clean routing, independent review,
adoption and the three-format manufacturing build. `./pcb build` refreshes accepted
layouts; `./pcb tools` exposes the focused operations and their help. Tool code and
tests live in `tools/pcbflow/`; this skill owns engineering/tool-selection guidance.

The repository layout profile owns the default copper-pour parameters and no-pour/with-pour policy. Keep every design's matching primary PCB routing-only and require it to pass DRC with zero unconnected items; a zone must never repair an incomplete routing system. During manufacturing generation, export the primary board as the default package, derive a single with-pour board under `variants/`, refill and DRC an isolated staging copy, and export that package separately. Generate both by default. A design that intentionally has no suitable pour net or must remain routing-only may set only the narrow `manufacturing.copper_pour_variants.enabled: false` exception in `.konnect/project.json`, with a reason.
