...
class WorkflowTests(unittest.TestCase):
    ...
    def test_discovers_design_and_builds_release_plan(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                ["design-contract", "design-check", "manufacturing-audit", "diff-check", "repository-growth"],
            )
    ...
    def test_pcb_review_uses_uncached_project_scoped_phases(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            design = self.make_repo(root)
            phases = workflow.phase_plan(root, "pcb-review", False, [design])
            self.assertEqual([p.name for p in phases], ["design-contract", "pcb-review-Example"])
            self.assertFalse(phases[-1].cacheable)
            self.assertIn(str(design / "Example.kicad_pcb"), phases[-1].command)

    @mock.patch.object(workflow.subprocess, "run")
    def test_review_receipt_keeps_artifact_paths(self, run):
        run.return_value = mock.Mock(returncode=0, stdout=json.dumps({
            "status": "pass", "artifacts": {"png": "review.png"}
        }), stderr="")
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            result = workflow.run_phase(workflow.Phase("pcb-review-Example", ("review",)), root, root / "log")
            self.assertEqual(result["evidence"]["artifacts"]["png"], "review.png")


    def test_manufacturing_cache_tracks_custom_rules(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            design = self.make_repo(root)
            phase = workflow.Phase("manufacturing-audit", cacheable=True)
            first = workflow.fingerprint(root, [design], phase)
            (design / "Example.kicad_dru").write_text('(version 1)')
            self.assertNotEqual(first, workflow.fingerprint(root, [design], phase))
