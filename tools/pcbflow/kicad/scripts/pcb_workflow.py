...
GOALS = ("pcb-review", "schematic-validate", "validate", "manufacturing-refresh", "release-ready")
...
def phase_plan(
    ...
    if goal == "pcb-review":
        reviewer = scripts / "headless_pcb_review.py"
        for design in designs:
            phases.append(Phase(
                f"pcb-review-{design.name}",
                (sys.executable, str(reviewer),
                 str(design / f"{design.name}.kicad_pcb")),
            ))
        return phases
    ...
    if goal == "release-ready":
        phases.append(Phase("design-check", (
            sys.executable, str(scripts / "check_design.py"),
            "--root", str(root / "Designs"), *selection,
        )))
    ...
    for design in designs:
        ...
        for candidate in design.rglob("*"):
            if not candidate.is_file() or any(part in EXCLUDED_DIRS for part in candidate.parts):
                continue
            if candidate.name == "PACKAGE.md" or candidate.suffix in {".kicad_pro", ".kicad_dru", ".kicad_pcb", ".gtl", ".gbl", ".gm1", ".gm2", ".drl", ".dxf", ".gbr", ".gbrjob"}:
...
def run_phase(phase: Phase, root: Path, log_path: Path) -> dict:
    ...
    if phase.name == "design-check" or phase.name.startswith("pcb-review-"):
        try:
            ...
            outcome["status"] = "fail"
            outcome["error"] = "invalid design-check or PCB review evidence"
            outcome["failure_class"] = "validation"
            outcome["next_action"] = "inspect the phase log"
...
def execute(
    ...
    for item in outcomes:
        if item['name'] == 'design-check' and 'evidence' in item:
            result['checks'] = item['evidence'].get('checks', [])
    if goal == "pcb-review":
        result["reviews"] = {
            item["name"]: item["evidence"] for item in outcomes if "evidence" in item
        }
