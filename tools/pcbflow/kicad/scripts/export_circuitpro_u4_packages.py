...
from manufacturing_discovery import drc_failure_detail, discover_packages, stage_package_board
...
def run(command: list[str]) -> None:
    ...
    if result.returncode:
        detail = ((result.stderr or result.stdout).strip() or "no diagnostic output") + drc_failure_detail(command)
...
def replace(packages: list[dict], generated: dict[str, Path], force: bool, clean_stale: bool) -> Path | None:
    ...
    return replace_directories(
        packages, generated, force=force, stale=stale, kind="circuitpro-u4",
    )
