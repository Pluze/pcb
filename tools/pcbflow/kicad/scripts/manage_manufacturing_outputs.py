...
def main() -> int:
    ...
    # Derive source variants first; install no production directory until every
    # selected design and all three formats have passed their staging validators.
    with tempfile.TemporaryDirectory(prefix="pcb-build-") as directory:
        staging_root = Path(directory)
        packages, generated = [], {}
        for design in designs:
            try:
                variant_command = [sys.executable, str(script_dir / VARIANT_GENERATOR), args.mode, str(design)]
                ...
                target = design
                if args.mode == "export":
                    target = staging_root / design.name
                    shutil.copytree(design, target, ignore=shutil.ignore_patterns(
                        'fabrication', 'archive', '.history', '*.lck', '*.kicad_prl'))
                for exporter in EXPORTERS:
                    command = [sys.executable, str(script_dir / exporter), args.mode, str(target)]
                    if args.mode == "export":
                ...
                if args.mode == "export":
                    for kind in ("u4", "lightburn", "gerber"):
                        key = design.name + "/" + kind
                        packages.append({"name": key, "output": design / "fabrication" / kind})
                        generated[key] = target / "fabrication" / kind
            finally:
                cleanup_variant_personal_state(design)
        if args.mode == "export":
            backup = replace_directories(packages, generated, force=args.force, stale=[], kind="manufacturing")
            if backup:
                print(f"BACKUP: {backup}")
            print(f"INSTALLED: {len(designs)} designs x 3 validated manufacturing formats")
